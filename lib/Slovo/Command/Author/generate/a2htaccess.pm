...
sub run ($self, @args) {
  ...
  unless ($cgi_script) {
    ...
    $cgi_file   = $home->list_tree({max_depth => 1})->first(qr/$cgi_script$/);
...
# Note!! If you run again `slovo generate a2htaccess` this file will be rewritten.
...
#<FilesMatch "(templates/|etc/|lib/|log/|t/|_build/|cover_db/|\.(pm|ep|conf|log|t|bak|yml|sqlite|sql)|READM.+|MANIF.+|Build.PL|Build|Makefi.+)$">
#    #2.2 configuration:
...
#    # Do not allow slovo responses to be cached unless they explicitly send cache
#    # headers themselves.
#    ExpiresActive Off
#  </FilesMatch>
#</IfModule>
