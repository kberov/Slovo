...
function get_страници ($url, $parent) {
    ...
    // alert('$url_no_qs:'+$url_no_qs);
    $.getJSON($url).done(function($data){
        // alert($('a:first-child',$parent).attr('href'));
        if($data.length === 0) {
        ...
        $data.forEach(function($row){
            let $page_url = `/${$row.alias}.стр.html`;
            let $page_link = `<a href="${$page_url}">${$row.title}</a>`;
            let $expander_url = '',
                $expander_link =  '',
                $item = `<div>${$page_link}${$expander_link}</div>`;
            if($row.is_dir === 1) {
                $expander_url = $url_no_qs + '?pid=' + $row.id;
                $expander_link = ` <a href="${$expander_url}" class="folder-expander mui--pull-right">☰</a>`;
                $item = `<strong>${$page_link}${$expander_link}</strong>`;
            }
            let $li = $('ul', $parent).append(`<li>${$item}</li>`);
             $('.folder-expander', $li).on('click', folder_expaner_onclick);
            //alert($li.html())
        });
    });
}
